...
io.on('connection', (socket) => {
  ...
  socket.on('audio-chunk', ({ meetingId, audioChunk, timestamp, recordingStartTime }) => {
    ...
    // ── FIX: Store recordingStartTime on the chunk ────────────────────────
    // Use the client-provided value; fall back to timestamp if missing
    // (handles old clients gracefully without breaking anything).
    const effectiveStartTime = recordingStartTime || timestamp || Date.now();
    // ─────────────────────────────────────────────────────────────────────
    ...
    queue.push({
      ...
      userName: displayName,
      timestamp: timestamp || Date.now(),
      recordingStartTime: effectiveStartTime,   // ← FIX: stored per chunk
      audioBuffer: Buffer.from(audioChunk),
    });
